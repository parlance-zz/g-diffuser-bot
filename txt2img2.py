...
# k_diffusion is used for alternative samplers
try: # install k_diffusion module if we haven't already
    import k_diffusion as K
except ImportError:
    print("Could not import k_diffusion, installing with pip...")
...
class KDiffusionSampler:
    ...
    def sample(self, S, conditioning, batch_size, shape, verbose, unconditional_guidance_scale, unconditional_conditioning, eta, x_T):
        sigmas = self.model_wrap.get_sigmas(int(S))
        x = x_T * sigmas[0]
...
def main():
    ...
    start_code = None
    if opt.fixed_code or fancy_sampler:
            ...
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        has_nsfw_concept = np.zeros(len(x_checked_image), dtype=np.int32)
